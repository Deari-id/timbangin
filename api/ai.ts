...
const clampStringArray = (value: unknown, maxItems: number, maxLength: number): string[] => {
...
const validateDecisionInput = (value: unknown): DecisionInput => {
  if (!value || typeof value !== 'object') {
    throw new Error('Invalid decision input');
  }

  const raw = value as Partial<DecisionInput>;
  const frameworks: DecisionFramework[] = ['General', 'Christianity', 'Muslim', 'Hedonistic', 'Efficient'];
  const framework = frameworks.includes(raw.framework as DecisionFramework) ? raw.framework as DecisionFramework : 'General';

  const alternatives = Array.isArray(raw.alternatives)
    ? raw.alternatives
        .slice(0, 3)
        .map((alt, index) => ({
          id: typeof alt?.id === 'number' ? alt.id : index + 1,
          text: trimToLimit(alt?.text, 500),
          userPros: clampStringArray(alt?.userPros, 8, 200),
          userCons: clampStringArray(alt?.userCons, 8, 200),
        }))
        .filter((alt) => alt.text)
    : [];

  const input: DecisionInput = {
    situation: trimToLimit(raw.situation, 1000),
    problem: trimToLimit(raw.problem, 1000),
    values: clampStringArray(raw.values, 3, 80),
    alternatives,
    framework,
  };

  if (!input.problem || input.alternatives.length === 0) {
    throw new Error('Problem and at least one alternative are required');
  }

  return input;
};

const generateAlternatives = async (problemContextRaw: unknown): Promise<string[]> => {
  const problemContext = trimToLimit(problemContextRaw, 1200);
  ...
  return clampStringArray(data.alternatives, 3, 250);
};

const analyzeDecision = async (rawInput: unknown): Promise<DecisionAnalysis> => {
  ...
  if (!response.text) {
    throw new Error("No response text received from AI. This might be due to safety filters.");
  }

  const data = JSON.parse(cleanJsonString(response.text)) as DecisionAnalysis;
...
const chatAboutAlternative = async (body: Record<string, unknown>): Promise<string> => {
  const problem = trimToLimit(body.problem, 1000);
  const alternative = trimToLimit(body.alternative, 500);
  const framework = trimToLimit(body.framework, 80) || 'General';
  const message = trimToLimit(body.message, 1000);
  const history = Array.isArray(body.history)
    ? body.history.slice(-10).map((h) => ({
        role: h?.role === 'model' ? 'model' : 'user',
        text: trimToLimit(h?.text, 1000),
      })).filter((h) => h.text)
    : [];

  if (!problem || !alternative || !message) {
    throw new Error('Problem, alternative, and message are required');
  }
...
const generateFollowUp = async (body: Record<string, unknown>): Promise<FollowUpAdvice> => {
  const problem = trimToLimit(body.problem, 1000);
  const alternative = trimToLimit(body.alternative, 500);
  const framework = trimToLimit(body.framework, 80) || 'General';

  if (!problem || !alternative) {
    throw new Error('Problem and alternative are required');
  }
  ...
  return JSON.parse(cleanJsonString(response.text)) as FollowUpAdvice;
};

export default async function handler(req: any, res: any) {
  if (!assertBodySizeAllowed(req, res) || !assertRequestAllowed(req, res)) {
    return;
  }
  ...
  try {
    ...
    let result: unknown;
    if (action === 'generateAlternatives') {
      result = await generateAlternatives(body.problemContext);
    } else if (action === 'analyzeDecision') {
      result = await analyzeDecision(body.input);
    } else if (action === 'chatAboutAlternative') {
      result = await chatAboutAlternative(body);
    } else if (action === 'generateFollowUp') {
      result = await generateFollowUp(body);
    } else {
    ...
    const message = error instanceof Error && error.message === 'AI service is not configured'
      ? error.message
      : 'AI request failed';
    res.status(message === 'AI service is not configured' ? 503 : 500).json({ error: message });
  }
}
